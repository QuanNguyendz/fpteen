-- Fixes / hardening for pickup scheduling
...
BEGIN
  ...
  LOOP
    ...
    -- UX: không trả về slot nằm trong quá khứ so với p_from.
    IF v_slot_start < p_from THEN
      CONTINUE;
    END IF;
...
DECLARE
  ...
  -- widen horizon to prevent "No available slots" for short spikes
  v_horizon INTERVAL := INTERVAL '24 hours';
BEGIN
  ...
  -- Compute the first candidate slot start (ceiling to slot boundary in store local time).
  ...
  FOR i IN 0..240 LOOP
    ...
    -- Horizon
    IF v_slot_start > (now() + v_horizon) THEN
      EXIT;
    END IF;
